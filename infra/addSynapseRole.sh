...
# Add Workspace Admin
workspaceAdminResponse=$(curl --request POST \
...
if [[ "$workspaceAdminResponse" == *"200"* ]]; then
  echo $workspaceAdminResponse
else
	exit 1
fi

# Add SQL Admin
sqlAdminResponse=$(curl --request POST \
  --url "https://$WORKSPACE_NAME.dev.azuresynapse.net/rbac/roleAssignments?api-version=2020-02-01-preview" \
  --header "authorization: Bearer $token" \
  --header 'content-type: application/json' \
  --write-out %{http_code} \
...
if [[ "$sqlAdminResponse" == *"200"* ]]; then
  echo $sqlAdminResponse
else
	exit 1
fi

# Add Spark Admin
sparkAdminResponse=$(curl --request POST \
  --url "https://$WORKSPACE_NAME.dev.azuresynapse.net/rbac/roleAssignments?api-version=2020-02-01-preview" \
  --header "authorization: Bearer $token" \
  --header 'content-type: application/json' \
  --write-out %{http_code} \
...
if [[ "$sparkAdminResponse" == *"200"* ]]; then
  echo $sparkAdminResponse
else
	exit 1
fi
